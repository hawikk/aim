...
finding carries `decision: "observe"` (locked policy decision). This is
...
as a post-ingest queue consumer; stdin mode is the pilot and
local-replay path.

Post-ingest Postgres mode: `evaluate-db` pulls events not yet in
...
Unattended compose form: `poll` runs `evaluate-db` on an interval so
...
Health endpoints: poll mode also serves `/healthz` (always 200) and
...
Alert delivery: findings that were newly inserted by a batch are
...
- **Google Chat, email (SMTP)** — one Cards V2 POST per batch to a space
...
- **PagerDuty** — Events API v2 trigger per finding.
...
- **Slack** — optional Block Kit POST per batch. **Feature-flagged
...
App-LLM new-sources → SOC: after each evaluate-db pass the runner
...
Provider/model catalogue drift → SOC: the same evaluate-db pass then
...
Restricted-repo matching: the `restricted-repo-access` rule matches
...
- `src/guardrail/new_sources.py` — App-LLM new-sources signal → finding; first-ever proxy provider-API `(host_ref, provider)` inside lookback becomes an observe finding delivered on the path
- `src/guardrail/catalogue_drift.py` — App-LLM catalogue completeness alerts; first-ever uncatalogued provider / unpriced model → Low finding on the path
- `src/guardrail/notify.py` — webhook + Sentinel notifiers, HMAC signing, retry policy
- `src/guardrail/notify.py` — webhook + Sentinel + Google Chat + optional Slack + PagerDuty notifiers, HMAC signing, retry policy
- `src/guardrail/escalation.py` — multi-stage escalation policies with timers
- `src/guardrail/poller.py` — resilient interval poller (the compose service)
- `src/guardrail/health.py` — stdlib /healthz + /readyz server for poll mode
