...
ingest receiver speaks OTLP/HTTP JSON only; the official Python OTel SDK's
HTTP exporter is protobuf and is rejected with 415.

Privacy: only the allowlisted gen_ai.* attributes and service.name are ever
put on the wire — never prompt/response text.
...
Configuration (all optional; unset endpoint = telemetry disabled, no-op):
  ...
  AIM_OTEL_SERVICE_NAME        service.name override (default: gatehouse)
"""
...
DEFAULT_SERVICE_NAME = "gatehouse"
