...
3. **Adversarial corpus + CI floors** — rates are measured and non-regressed (AIM-579 floors: baseline 100 / evasion ≥90 / FP 100).
...
## When CEO would revisit

Reopen this ADR only if **all** of the following hold:

1. Explicit CEO/Security/Legal decision that content inspection is in scope for a named use case.
2. DPIA / works-council path for that use case (AIM-59 lineage).
...
Until then, competitive pressure alone is **not** a reason to ship prompt-shipping classifiers. Research §9 “Do not chase” on AIM-502 stands.
...
| Buy NeuralTrust/Noma for semantic depth | Different product category (gateway); only if app-LLM **enforcement** is in scope (AIM-603) |

## References

- `docs/security/detector-evasion-capability.md` (generated rates)
- `docs/security/block-mode-precision-gates.md`
- Competitive research: AIM-502 document `competitive-review` §6.5 / §9
