/* Hash-router core (extracted, module views).
 *
 ...
 * unit-tested (see apps/web/test/router.test.js). The dead end lived
 ...
 * bare `return`.
 *
 * feature modules (findings/rules/compliance/mcp/onboarding) used to
 ...
 * with the router about what was on screen; the dead end was one
...
// `runbooks` is a module view: `#/runbooks/<slug>` deep-links a SOC
 ...
 *
 * `drill: true` keeps the second path segment as `entity` so module
 ...
 * current: no event will fire, so the caller owns the re-render. the
 ...
 *
 * For the two module landings made explicit: first-run onboarding and
