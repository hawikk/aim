...
Same configuration contract as the claude-code collector: a managed JSON
file dropped by endpoint tooling (Intune/SCCM on Windows, config-management
on Linux/WSL), with per-user or per-run env vars overriding file values.
...
> ``token`` field. A plaintext ``token`` key is accepted for local
development only; managed deployments use an ACL-protected token file.
"""
