#!/usr/bin/env python3
"""AIM-117 enforcement bake driver.
...
so ``scripts/aim-110-shadow-report.mjs`` aggregates them.

Why a backtest and not only live-hook capture: AIM-110/111 shipped the hooks,
...
Honesty contract:
  ...
  - Metadata-only, fail-open posture unchanged: only action + rule_id +
    ...
    discarded (same as personal mode, AIM-77).
...
Usage:
  python scripts/aim-117-shadow-bake.py [--since-days N] [--dry-run]
