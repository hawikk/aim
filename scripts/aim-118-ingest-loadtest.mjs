#!/usr/bin/env node
/**
 * AIM-118 — Ingest scale proof: 700-engineer burst load test.
 *
 ...
 *
 * Usage:
 *   INGEST_TOKEN=... node scripts/aim-118-ingest-loadtest.mjs
 * Env:
...
async function postEvents(events, ids = null) {
  ...
  try {
    ...
    // (AIM-127) as well as the original queue-and-hold behaviour.
...
// ---- main ---------------------------------------------------------------
const report = { issue: "AIM-118", startedAt: new Date().toISOString(), base: BASE, phases: {} };

const model = deriveModel(MODEL);
report.model = model;
console.log("\n=== AIM-118 ingest scale proof ===");
...
if (PHASES.includes("D")) {
  ...
  // any shedding to this phase (AIM-127). Null when admission is off / metrics
