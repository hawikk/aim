#!/usr/bin/env node
/**
 * canary end-to-end: secret-in-prompt detection, collector flags →
...
try {
  ...
  if (finding) {
    const patch = await fetch(`${API_URL}/api/findings/${finding.findingId}`, {
      ...
      body: JSON.stringify({ status: "acknowledged", note: "Canary: detector verified end-to-end" }),
    });
    ...
    check("triage persisted in Postgres", persisted.startsWith("acknowledged canary"),
      persisted);
  }
