#!/usr/bin/env node
/**
 * AIM-97 staging e2e: per-tool-call guardrail rules fire end-to-end and the
 * new-MCP-server alert is delivered through the AIM-76 webhook path.
 *
 ...
 *
 * Prereqs (same as scripts/aim-32-canary-e2e.mjs):
 ...
 *
 * Usage:
 *   node scripts/aim-97-tool-policy-e2e.mjs
 *
...
try {
  ...
  // findings carry the HMAC pseudonym, exactly like production (AIM-78).
  ...
  writeFileSync(join(stagingRulesDir, "staging-overlay.yaml"),
    `# AIM-97 staging overlay — NOT shipped policy. Activates the restricted-repo
...
`);
  ...
  // ---- 7. webhook delivery (AIM-76 path): signature + payload + audit row --
  {
    ...
    if (hook) {
      ...
      check("new-MCP-server alert delivered via webhook (AIM-76)", Boolean(rec),
  ...
  writeFileSync(MEASUREMENT_OUT, JSON.stringify({
    ...
    pipeline: "ingest API -> guardrail evaluate-db (staging ruleset overlay) -> findings + webhook (AIM-76)",
