#!/usr/bin/env bash
# AIM-292 — product evidence: clean machine → real findings (timed).
#
...
#
# Usage:
#   ./scripts/aim-292-product-evidence.sh
#   ./scripts/aim-292-product-evidence.sh --keep
#   AIM_292_MODE=fixture ./scripts/aim-292-product-evidence.sh   # CI-friendly
#
# Optional env:
...
INSERT INTO findings (
  ...
  'critical',
  'Secret pattern detected in AI usage (AIM-292 evidence)',
...
(
  ...
  git init -q -b main
  cat > main.tf <<'TF'
# AIM-292 PR-gate canary (intentionally insecure IaC for scanner proof)
...

{
  ...
  echo
  echo '```sh'
  echo './scripts/aim-292-product-evidence.sh'
  echo '```'
} >"$REPORT"
...
echo ""
echo "=========================================="
echo "AIM-292 product evidence complete"
