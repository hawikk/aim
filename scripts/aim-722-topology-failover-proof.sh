#!/usr/bin/env bash
# AIM-722 — multi-AZ topology + proven failover (composite proof).
#
...
#
# Usage: ./scripts/aim-722-topology-failover-proof.sh
# Env:
...
echo "== AIM-722 multi-AZ topology + failover proof =="
echo "root: $ROOT"
echo ""

# --- 1. Helm topology render (required) ---
echo "---- [1/2] helm enterprise topology render ----"
if ! "$ROOT/scripts/aim-769-topology-render-proof.sh"; then
...
if [[ "${SKIP_HA:-0}" == "1" ]]; then
  ...
  if ! docker inspect "$PRIMARY" >/dev/null 2>&1; then
    ...
    set +e
    "$ROOT/scripts/aim-553-ha-smoke.sh" 2>&1 | tee "$HA_LOG"
    HA_RC=${PIPESTATUS[0]}
    set -e
    if [[ $HA_RC -eq 0 ]]; then
...
if [[ $FAILURES -eq 0 ]]; then
  echo "RESULT: PASS ($FAILURES failures) — AIM-722 topology + failover evidence"
  exit 0
else
