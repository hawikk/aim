#!/usr/bin/env node
/**
 * AIM-110 shadow-mode would-block report. Aggregates the endpoint
 ...
 *
 * Usage:
 *   node scripts/aim-110-shadow-report.mjs > docs/aim-110-shadow-report.md
 *   node scripts/aim-110-shadow-report.mjs --self-test
 *
 ...
 *  a pre-AIM-110 collector: both are invisible to the bake and both are worth
 *  naming even when the overall verdict is fine. */
export function coverageGaps(c) {
  const gaps = [];
  const dark = c.endpointsTotal - c.endpointsAware;
  if (dark > 0) {
    gaps.push(`${dark} endpoint(s) report telemetry but emit no posture marker — pre-AIM-110 collector build. `
...
async function main() {
  ...
  const lines = [
    `# AIM-110 endpoint enforcement — shadow-mode would-block report`,
    "",
    `Source: \`${DB_NAME}\`, events since ${SINCE}. Generated ${new Date().toISOString().slice(0, 10)} by \`scripts/aim-110-shadow-report.mjs\`.`,
    "",
